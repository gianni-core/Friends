...
app.use(session({
    secret: 'secret_key_gianni_secure', // Change pour la prod
    resave: false,
...
db.serialize(() => {
    db.run(`CREATE TABLE IF NOT EXISTS users (
        ...
        bonte INTEGER DEFAULT 50,
        humour INTEGER DEFAULT 50,
        sociabilite INTEGER DEFAULT 50,
        confiance INTEGER DEFAULT 50,
    ...
    db.get("SELECT * FROM users WHERE username = ?", ['Gianni'], (err, row) => {
        if (!row) {
            ...
            db.run(`INSERT INTO users (username, password, role, is_public) VALUES (?, ?, ?, ?)`, 
                ['Gianni', hash, 'admin', 1]);
            console.log("Compte Admin 'Gianni' créé.");
        }
    });
});
...
app.get('/api/me', auth, (req, res) => {
    db.get("SELECT id, username, role, is_public, bonte, humour, sociabilite, confiance, respect, dopamine FROM users WHERE id = ?", [req.session.user.id], (err, row) => res.json(row));
});
...
app.get('/api/admin/users', admin, (req, res) => {
    db.all("SELECT id, username, role, is_public, bonte, humour, sociabilite, confiance, respect, dopamine FROM users", (err, rows) => res.json(rows));
});
...
app.delete('/api/admin/delete/:id', admin, (req, res) => {
    if (req.params.id == req.session.user.id) return res.status(400).json({ error: "Impossible de se supprimer soi-même" });
